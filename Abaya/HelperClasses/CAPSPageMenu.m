            ...
            if (scrollView.contentOffset.x >= 0.0 && scrollView.contentOffset.x <= ((CGFloat)(_controllerArray.count - 1) * self.view.frame.size.width)) {
                ...
                if ((_currentOrientationIsPortrait && UIInterfaceOrientationIsPortrait(orientation)) || (!_currentOrientationIsPortrait && UIInterfaceOrientationIsLandscape(orientation))){
                    ...
                    if (!_didTapMenuItemToScroll) {
                        ...
                        NSString *sesstioncheck = [[NSUserDefaults standardUserDefaults] stringForKey:@"session"];
                        if ([sesstioncheck isEqualToString:@"session"])
                        {
                            [[NSUserDefaults standardUserDefaults] setObject:nil forKey:@"session2"];
                            [[NSUserDefaults standardUserDefaults] setObject:nil forKey:@"timestamps"];
                        }
                        else{
                            NSUserDefaults *standardUserDefaults = [NSUserDefaults standardUserDefaults];
                            
                            if (standardUserDefaults) {
                                [standardUserDefaults setObject:@"session2" forKey:@"session2"];
                                [standardUserDefaults setObject:[NSString stringWithFormat:@"%ld",(long)_currentPageIndex] forKey:@"timestamps"];
